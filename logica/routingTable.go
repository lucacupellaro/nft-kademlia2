...
func JoinAndExpandLite(ctx context.Context, seederAddr, selfName string, alpha, seedSample, iters int) error {
	...
	for it := 0; it < iters; it++ {
		...
		for _, src := range live {
			for _, tgt := range targets {
				...
				go func() {
					...
					mu.Lock()
					//calcaola la distanza XOR e aggiungi a prospects
		...
		//pingo i prospects più vicini a target che non sono già in RT
		nowSet := kbSnapSet(kBucketPath)
		toPing := make([]string, 0, KCapacity)
		for _, p := range prospects {
			if len(toPing) >= KCapacity {
				break
			}
			nm := p.Name
